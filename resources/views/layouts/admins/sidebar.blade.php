...
<div x-cloak
    class="fixed inset-y-0 left-0 z-20 w-64 transition-all duration-300 transform bg-gray-900 lg:translate-x-0 lg:static lg:inset-0 flex flex-col"
    :class="{ 'translate-x-0 ease-out shadow-xl': sidebarOpen, '-translate-x-full ease-in': !sidebarOpen }">

    <!-- Header & Navigation Container - akan grow untuk push logout ke bawah -->
    <div class="flex-1 overflow-y-auto">
        ...
        <nav class="mt-10" style="scrollbar-width: thin;">
            <!-- Dashboard Link -->

            <!-- Data Sekolah Link -->
            <a class="flex items-center px-6 py-2 mt-4 transition-all duration-200 ease-in-out transform {{ Route::currentRouteNamed('admin.schoolData') ? 'text-gray-100 bg-gray-700 bg-opacity-25 scale-105' : 'text-gray-500 hover:scale-105' }} hover:bg-gray-700 hover:bg-opacity-25 hover:text-gray-100"
                href="{{ route('admins.schoolData') }}">
    ...
    <!-- Logout Button - akan selalu di bottom -->
    <div class="flex-shrink-0 w-full p-2 bg-red-600 text-center text-white hover:bg-red-700 transition-colors duration-200 cursor-pointer">
